...
const cli = async (processArgv) => {
  ...
  const wantsCompiled = processArgv.indexOf('--compiled') >= 0;
  ...
  if (command === 'completion') {
    ...
    if (devMode && !wantsCompiled) {
      ...
      const TSCli = require(`${coreData.corePath}/src/cli/cli`);
      (new TSCli(coreData)).start()
    }
    else {
      const Cli = require(`${coreData.corePath}/lib/cli/cli`);
...
const prepare = async (command, config) => {
  ...
  if (fse.existsSync(path.join(config.moduleRoot, config.moduleName))) {
    coreData['modules'] = await readJson(path.join(config.moduleRoot, config.moduleName))
  }
