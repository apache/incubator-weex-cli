...
const { getNodeArgs, readJson, install, confirm } = require('../lib/util');
...
const cli = async (processArgv) => {
  ...
  else if (wantsCompiled) {
    // compiled the command
  }
  else {
...
const prepare = async (command, config) => {
  ...
  // check if there has weex-cli/core
  ...
  else if (corePackageJson.name && corePackageJson.version) {
    try {
      ...
      if (latest && latest.body) {
        ...
        if (semver.gt(coreVersion, corePackageJson.version)) {
          const result = await confirm(
            'Core has a new version ' + colors.green(coreVersion) +
