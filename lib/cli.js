...
// options for cli/core
const cliOptions = {
  plugins: {
    value: path.join(__dirname, '../packages/@weex/plugins/'),
    options: {}
  }
}
...
const cli = async (processArgv) => {
  ...
  const devMode = fse.existsSync(`${coreConfigs.corePath}/src`);
  ...
  if (command === 'repair' || command === 'doctor' || command === 'config') {
    ...
    if (devMode && !wantsCompiled) {
      ...
      require(`${coreConfigs.corePath}/src/cli/cli`).run(process.argv, cliOptions)
    }
    else {
      require(`${coreConfigs.corePath}/lib/cli/cli`).run(process.argv, cliOptions)
    }
  }
}
