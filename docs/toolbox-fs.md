...
You can access these tools on the `@weex-cli/core`, via `const { fs } = require('@weex-cli/core')`.
...
```js
toolbox.fs.separator // '/' on posix but '\' on windows
```
...
```js
toolbox.fs.homedir() // '/Users/jh' on my macOS machine
```
...
```js
toolbox.fs.subdirectories(`~/Desktop`) // []
```
